# -*- coding: utf-8 -*-
"""
Created on Fri Jul  5 10:53:44 2024
...
class TensorRTInfer:
    ...
    def __init__(self, engine_path,BATCH_SIZE):
        ...
        for i in range(self.engine.num_bindings):
            ...
            shape[0]= BATCH_SIZE         
            if is_input and name=='batched_input':
    ...
    def set_dynamic_shape(self, input_shape):
        """
        Update the dynamic input shape for the model.
        :param input_shape: New input shape to set for the model.
        """
        assert len(input_shape) == len(self.inputs[0]['shape']), "Input shape must match the model input dimensions"
        self.context.set_binding_shape(self.inputs[0]['index'], input_shape)
        self.inputs[0]['shape'] = input_shape
    def input_spec(self):
        """
    ...
    def infer(self,BATCH_SIZE, batch,N_W_RUN,N_RUN,test_batches_label):
        ...
        self.set_dynamic_shape(batch.shape)
        # Allocate memory for output dynamically
        shape_out, dtype_out = self.output_spec()
        output_shape = self.context.get_binding_shape(self.outputs[0]['index'])
        output = np.zeros(output_shape, dtype_out)
        ...
        #segformer
        output = np.zeros((test_batches_label.shape[0],n_labels,56,56), dtype_out) 
        ...
        logits = tf.image.resize(logits, [IMAGE_SIZE,IMAGE_SIZE], antialias=True)
        #print(logits.shape)
        predictions = tf.math.argmax(logits, axis=-1)
        #predictions = np.squeeze(predictions)
        #print(predictions.shape) 
        #print(predictions)    
        #print(test_batches_label.shape)
...
def main():
  ...
  trt_infer = TensorRTInfer(args.model, BATCH_SIZE)
  ...
  nb_modelo = args.model.split("_")[2] # modificcado [2]
