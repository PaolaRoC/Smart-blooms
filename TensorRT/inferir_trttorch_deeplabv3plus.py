# -*- coding: utf-8 -*-
"""
Created on Mon Jun 24 10:57:55 2024
...
class TensorRTInfer:
    ...
    def infer(self,BATCH_SIZE, batch,N_W_RUN,N_RUN,test_batches_label):
        ...
        """
        print(round((keras_accuracy_ver[0].result().numpy()*100),3))
        print(round((keras_accuracy_ver[1].result().numpy()*100),3))
        print(round((keras_accuracy_ver[2].result().numpy()*100),3))
        print(round((keras_accuracy_ver[3].result().numpy()*100),3))
        print(round((keras_accuracy_ver[4].result().numpy()*100),3))
        print(round((keras_accuracy_ver[5].result().numpy()*100),3))
        """
...
def main():
  ...
  import pandas as pd 
  DF = pd.DataFrame(matriz_info) 
  
  # save the dataframe as a csv file 
  DF.to_csv("/homelocal/yadirapr_local/CSV_info/resnetfp32.csv")
