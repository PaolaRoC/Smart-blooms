...
class DenseNet201(nn.Module):
    def __init__(self, output_layer=None):
        ...
        self.pretrained = models.densenet201(weights=models.DenseNet201_Weights.IMAGENET1K_V1)
        self.output_layer = output_layer
        self.layers = list(self.pretrained.features._modules.keys())
        self.layer_count = 0
        for l in self.layers:
            if l != self.output_layer:
                self.layer_count += 1
                ...
                break
        for i in range(1, len(self.layers)-self.layer_count):
            self.dummy_var = self.pretrained.features._modules.pop(self.layers[-i])
        self.net = nn.Sequential(self.pretrained.features._modules)
        self.pretrained = None
...
class EfficientNetB6(nn.Module):
    def __init__(self, output_layer=None):
        ...
        self.pretrained = models.efficientnet_b6(weights=models.EfficientNet_B6_Weights.IMAGENET1K_V1)
        self.output_layer = output_layer
        self.layers = list(self.pretrained.features._modules.keys())
        self.layer_count = 0
        for l in self.layers:
...
class Deeplabv3Plus(nn.Module):
    def __init__(self, num_classes, network_backbone):
        ...
        if network_backbone == "ResNet50":
            ...
            self.input_ch = 1024
            self.in_ch = 256
        elif network_backbone == "DenseNet201":
            
            self.backbone = DenseNet201(output_layer='denseblock3')
    
            self.low_level_features = DenseNet201(output_layer='denseblock1')
            
            self.input_ch = 1792
            self.in_ch = 256
            
        elif network_backbone == "EfficientNetB6":
            
            self.backbone = EfficientNetB6(output_layer='4')
    
            self.low_level_features = EfficientNetB6(output_layer='2')
            
            self.input_ch = 144
            self.in_ch = 40

        self.assp = ASSP(in_channles=self.input_ch, out_channles=256)

        self.conv1x1 = Atrous_Convolution(
            input_channels=self.in_ch, output_channels=48, kernel_size=1,
