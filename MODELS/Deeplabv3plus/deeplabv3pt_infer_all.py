...
def plot_samples_matplotlib(display_list, display_string, idx , outimg_path,figsize=(1, 3)):
    ...
    #plt.show()
    result_path = os.path.join( outimg_path, "deeplab3plus_pred_vs_ground_img_"+ str(idx) +".png")
    plt.savefig(result_path)


def evaluation_deeplabv3(model: nn.Module, device, valid_dataloader, metric,NUM_CLASSES, outimg_path ): 
    model.to(device)
    model.eval()
    ...
    for idx, batch in enumerate(valid_dataloader):
      pixel = batch["pixel_values"].type(torch.float).to(device)
      ...
      predicted_segmentation_map = outputs.argmax(dim=1)
...
def main():
    ...
    model_deeplab = Deeplabv3Plus(num_classes= NUM_CLASSES, network_backbone=NET_BACKBONE)
    model_deeplab.load_state_dict(torch.load(MODEL_path, map_location={'cuda:1': 'cuda:0'}), strict=False)
    
    
    print_trainable_parameters(model_deeplab)
    ...
    evaluation_deeplabv3(model_deeplab, device, valid_dataloader, metric, NUM_CLASSES, img_path )
