# -*- coding: utf-8 -*-
"""
Created on Thu Sep 26 08:53:53 2024
...
class SemanticSegmentationDataset(Dataset):
    ...
    def __init__(self, root_dir, image_processor, train=True,transform=None):
        ...
        list_dir= '/scratch/yadirapr_local/sam2/text'
        split_train = 'train_dataset2'
...
def plot_samples_matplotlib(display_list, display_string, idx ,outimg_path, figsize=(1, 3)):
    ...
    result_path = os.path.join( outimg_path, "samed_final"+ str(idx) +".png")
...
def evaluation_sam(sam_model: nn.Module, device, valid_dataloader, metric, NUM_CLASSES, outimg_path ): 
    ...
    for idx, batch in enumerate(valid_dataloader):
      ...
      with torch.no_grad():
        ...
        pixel_values = sam_model.preprocess(pixel_values)
        image_embedding = sam_model.image_encoder(pixel_values)
      ...
      predicted_segmentation_map = torch.argmax(torch.softmax(upscaled_masks, dim=1), dim=1)
    
    
      plots_pred.append(predicted_segmentation_map.cpu().numpy())
      plots_img.append(pixel_values.permute(0,2,3,1).cpu().numpy())
    ...
    outimg_path = r"/homelocal/yadirapr_local/SAMed/Out_img/SAMed_testv2/"
...
def main():
    ...
    MODEL_path = config_file["MODEL"]["LORA_PATH"] # "./SAMed/output/Cyano_224_pretrain_vit_b_epo404_bs12_lr0.001/epoch_319.pth"
    RANK = config_file["MODEL"]["RANK"]
    ...
    sam_model, img_embedding_size = sam_model_registry['vit_b'](image_size=224,
    ...
    lora_sam = LoRA_Sam(sam_model, RANK)

    lora_sam.load_lora_parameters(MODEL_path)

    sam_model = lora_sam.sam
    
    print_trainable_parameters(model=sam_model)
    ...
    evaluation_sam(sam_model, device, valid_dataloader, metric, NUM_CLASSES, img_path )
