# -*- coding: utf-8 -*-
"""
Created on Mon Nov 18 11:17:01 2024
...
class SemanticSegmentationDataset(Dataset):
    ...
    def __init__(self, root_dir, image_processor, train=True,transform=None):
        ...
        list_dir= './text'
        split_train = '/scratch/yadirapr/sam2/text/train_dataset2'
        split_test = '/scratch/yadirapr/sam2/text/validation_dataset2'
...
def plot_samples_matplotlib(display_list, display_string, idx , outimg_path, figsize=(1, 3)):
    ...
    #plt.show()
    result_path = os.path.join( outimg_path, "segformer_pred_vs_ground_img_"+ str(idx) +".png")
    plt.savefig(result_path)
    
def evaluation_segformer(model: nn.Module, device, valid_dataloader, metric,NUM_CLASSES, outimg_path):
    model.to(device)
    model.eval()
    info_p_cat_iou= []
    info_mean_iou = 0.0
    ...
    for idx, batch in enumerate(valid_dataloader):
      ...
      predicted_segmentation_map = image_processor.post_process_semantic_segmentation(outputs, target_sizes=[224])[0]
      predicted_segmentation_map = predicted_segmentation_map.cpu().numpy()
      plots_pred.append(predicted_segmentation_map)
      ...
      metrics = metric._compute(
                      ...
                      references=[np.squeeze(lab, axis=0)],
    ...
    random_img = np.random.choice(range(len(valid_dataloader)), 5, replace=False)
    random_img = [1,4,7]
    ...
    nombre_archivo = './resultados_cpu.csv'
...
def main():
    ...
    MODEL_path = config_file["MODEL"]["MODEL_PATH"] # "./segformer_b3v2_224_10_420_0001/epoch_400/"
    ...
    segformer = SegformerForSemanticSegmentation.from_pretrained(MODEL_path)

    print_trainable_parameters(segformer)
    ...
    img_path = os.path.join(PREDICTED_MASK_PATH , 'segformer')
    
    if not os.path.exists(img_path):
            os.makedirs(img_path)

    evaluation_segformer(segformer, device, valid_dataloader, metric,NUM_CLASSES, img_path)
